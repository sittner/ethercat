...
# check (script/rt-effects-check.sh).
#
...
# body verification).  Distributions commonly ship older clang, so the
# check uses a pinned official LLVM release binary, downloaded once into
# the user cache and verified against the sha256 digests published by
# the LLVM project on GitHub.  Only the compiler frontend and its
# resource headers are extracted (~370 MB); the tarball is deleted after
# extraction.
#
...
#
# Set RT_CLANG to a clang >= 20 binary to skip the download entirely.
#
...
if [ -n "${RT_CLANG:-}" ]; then
    if [ ! -x "$RT_CLANG" ]; then
        echo "rt-clang.sh: RT_CLANG=$RT_CLANG is not executable" >&2
        exit 1
    fi
    echo "$RT_CLANG"
    exit 0
...
CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/ethercat-rt-clang"
