    ...
    if (fsm->rescan_required) {
        ...
        if (!master->allow_scan) {
            ...
            if (!count) {
                ...
                master->scan_busy = 0;
                ec_pal_wake_up(&master->plat.scan_queue);
        ...
        )
{
    ...
    if (slave->sii.mailbox_protocols & EC_MBOX_EOE) {
        ...
        ec_eoe_t *eoe;
        if (!(eoe = ec_pal_malloc(sizeof(ec_eoe_t)))) {
        ...
        )
{
    ...
    ec_pal_wake_up(&master->plat.config_queue);
        ...
        )
{
    ...
    if (!ec_fsm_coe_success(&fsm->fsm_coe)) {
        ...
        request->state = EC_INT_REQUEST_FAILURE;
        wake_up_all(&fsm->master->plat.request_queue);
        ec_fsm_master_restart(fsm);
        return;
    }
