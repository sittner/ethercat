...
void ec_slave_clear(ec_slave_t *slave /**< EtherCAT slave */)
{
    ...
    if (slave->sii.strings) {
        for (i = 0; i < slave->sii.string_count; i++)
            ec_pal_free(slave->sii.strings[i]);
        ec_pal_free(slave->sii.strings);
    }
        ...
        )
{
    ...
    if (slave->sii.string_count) {
        if (!(slave->sii.strings =
                    ec_pal_malloc(sizeof(char *) * slave->sii.string_count))) {
        ...
        for (i = 0; i < slave->sii.string_count; i++) {
            ...
            if (!(slave->sii.strings[i] =
                        ec_pal_malloc(sizeof(char) * size + 1))) {
        ...
        )
{
    ...
    if (count) {
        ...
        if (!(syncs = ec_pal_malloc(memsize))) {
        ...
        )
{
    ...
    while (data_size >= 8) {
        if (!(pdo = ec_pal_malloc(sizeof(ec_pdo_t)))) {
